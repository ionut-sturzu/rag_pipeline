...
module "nim_nemo_embedder" {
  count  = var.deploy_nim_nemo_embedder ? 1 : 0
  source = "./helm-module"
  ...
  deployment_name     = "nemo-embedder"
  namespace           = "default"
  helm_chart_path     = "./text-embedding-nim"

  pre_deployment_commands = [
    "kubectl get secret -n default nvcr-${local.state_id} || kubectl create secret docker-registry -n default nvcr-${local.state_id} --docker-server=nvcr.io --docker-username='${var.nvcr_username}' --docker-password='%{if length(var.nvcr_password) > 0}${var.nvcr_password}%{else}${var.NGC_API_KEY}%{endif}'",
    "kubectl get secret -n default ngc-api || kubectl create secret generic -n default ngc-api --from-literal=NGC_CLI_API_KEY=${var.NGC_API_KEY}",
  ]
  deployment_extra_args = [
    "--timeout 10m0s"
  ]
  ...
  helm_template_values_override = templatefile(
    "${path.root}/helm-values-templates/nemo-embedder-values.yaml.tpl",
    {
      nvcr_secret               = "nvcr-${local.state_id}",
      embedder_image_repository = var.embedder_image_repository
      embedder_image_tag        = var.embedder_image_tag
    }
  )
  helm_user_values_override = try(base64decode(var.nemo_embedder_user_values_override), var.nemo_embedder_user_values_override)
